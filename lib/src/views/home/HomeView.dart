...
class HomeView extends StatelessWidget {
  ...
  List<RecommendedLeason> parseJSON(String res) {
    ...
    return parsed
        .map<RecommendedLeason>((json) => RecommendedLeason.fromJSON(json))
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    final recommendedLeasons = FutureBuilder(
      future: DefaultAssetBundle.of(context)
          .loadString('assets/json/recommendedLeasons.json'),
      builder: (context, snapshot) {
        List<RecommendedLeason> leasons = parseJSON(snapshot.data.toString());
        return leasons.isNotEmpty
            ? ListView(
                children: getCards(leasons), scrollDirection: Axis.horizontal)
            : SizedBox();
      },
    );
    return Column(
      ...
      children: [
        AppBanner(),
        SizedBox(height: 20),
        Container(height: 175, child: recommendedLeasons)
      ],
    );
  }
}
